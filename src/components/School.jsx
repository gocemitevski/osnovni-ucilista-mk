...
const School = (props) => {
  ...
  const [school, setSchool] = useState(() => {
    const schoolResult = props.data.filter((el) =>
      el.some((item) =>
        cleanName(transliterate(item.toString() + " " + el[2].toString()))
          .toLowerCase()
          .includes(schoolId)
      )
    );
  ...
  const [nearbySchools, setNearbySchools] = useState(() => {
    const nearbySchoolResults = props.data.filter((el) =>
      el.some(
        (item) =>
          school &&
          item.toString().toLowerCase().includes(school[2].toLowerCase())
      )
    );
          ...
          <div className="row justify-content-center">
            <div className="col-xl-8">
              <h2 className="mt-5 mb-4 text-center">
                Други училишта во <MunicipalityLink municipality={school[2]} />
                {school[7] && `, `}
                {school[7] && <MunicipalityLink municipality={school[7]} />}
              </h2>
