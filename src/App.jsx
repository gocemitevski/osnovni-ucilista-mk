...
function App() {
  ...
  const [skopjeSchoolsCount, setSkopjeSchoolsCount] = useState(() => {
    return data.records.filter(
      (el) => el[7] && el[7].toString().includes(skopjeTitle)
    ).length;
  });
  ...
  return (
    <HashRouter>
          ...
          <Route
            ...
            element={(props) => (
              <item.component
                ...
                title={item.title}
                data={data.records}
                municipalitiesSort={municipalitiesSort}
                pageWidth={pageWidth}
                skopjeTitle={skopjeTitle}
                skopjeSchoolsCount={skopjeSchoolsCount}
                setSocialIconLinks={setSocialIconLinks}
              />
            )}
          />
        ))}
      ...
      >
        ...
        искуство. Изберете „Во ред“ или „Не, благодарам“ за да го скриете ова
        предупредување.
      </CookieConsent>
    </HashRouter>
  );
